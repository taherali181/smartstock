...
- Purchase receipts: `POST /v1/purchase-orders/{order_id}/receipts` atomically records accepted and rejected quantities, updates PO progress, posts inventory/valuation, and generates putaway tasks. The request supplies the PO version and affected inventory-position versions. Warehouse execution uses `POST /v1/warehouse-tasks/{task_id}/receipt`, which completes the receiving task in the same transaction and creates follow-up receive work when the PO remains partially received.

...
The Phase 3 implementation also produces `purchase_order.created`, `purchase_order.state_changed`, `order.created`, `order.state_changed`, `warehouse_task.created`, and `warehouse_task.state_changed`. Atomic receipt posting emits `receipt.posted`, `inventory.ledger_posted`, and `inventory.position_changed`. Inventory-backed allocation emits `inventory.reservation_created`, `warehouse_task.created`, and canonical `order.allocation_changed`. Reservation-backed outbound execution emits canonical `shipment.shipped`. The required approval and confirmation transitions retain `purchase_order.approved` and `order.confirmed`.

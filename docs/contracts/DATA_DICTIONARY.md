...
| `warehouse_tasks` | Prioritized receiving, putaway, pick, pack, transfer, count, and replenishment work | tenant task number; warehouse grants; explicit execution state; count/transfer work pins stock identity and snapshot version; source transfer work identifies its destination warehouse |
...
| `return_authorizations` / `return_lines` / `return_receipts` | Versioned RMA decision lifecycle and immutable quarantine receipt | shipped-quantity ceiling; tenant order/warehouse/product FKs; original shipment cost |

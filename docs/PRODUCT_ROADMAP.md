...
Implementation status: in progress. The delivered slices add persisted purchase-order, sales-order, receipt, allocation, shipment, RMA, and warehouse-task aggregates with decimal lines, tenant-scoped command idempotency, optimistic versions, named transitions, warehouse grants, RLS, audit, outbox events, generated API contracts, and a live operational queue panel. Task-bound purchase receipt commands now post valued accepted/quarantined inventory and complete receiving work atomically, create version-bound putaway tasks, and generate follow-up receiving work for partial POs. Allocations reserve row-locked sellable positions; shipments consume exact reservations and post valued outbound inventory. RMAs validate shipped quantities, require authorization, receive at original shipment cost into quarantine, and separate inspection from refund/replacement/credit decisions. Count tasks bind a blind count to an exact stock snapshot and atomically post variance plus completion. Transfer tasks now ship source stock into an in-transit ledger account, generate destination receipt work, and resolve the full shipment into received stock plus any discrepancy. An installable Workbox PWA provides a responsive task queue, IndexedDB cache, ordered offline physical-command replay, explicit conflict review, and camera/hardware/manual barcode verification. The remaining Phase 3 work is reporting/notifications, offline receipt-line entry, putaway/pick/pack-specific execution, transfer discrepancy resolution, and the full device/browser exit gate.

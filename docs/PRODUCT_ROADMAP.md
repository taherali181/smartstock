...
Implementation status: in progress. The delivered slices add persisted purchase-order, sales-order, receipt, allocation, shipment, and warehouse-task aggregates with decimal lines, tenant-scoped command idempotency, optimistic versions, named transitions, warehouse grants, RLS, audit, outbox events, generated API contracts, and a live operational queue panel. Receipts atomically post accepted/quarantined valued inventory and generate putaway. Allocations reserve row-locked sellable positions and generate pick work. Shipments consume those exact reservations, post valued outbound ledger entries, and derive partial/full fulfillment state. Status-only allocation and shipment bypasses are not exposed. Returns, offline scanning, and the Phase 3 exit gates remain open.

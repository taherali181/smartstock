...
| Operations | Phase 3 order/task foundation with task-bound atomic purchase receiving, allocation, shipment, RMA, blind-count, and staged-transfer execution: valued inbound/outbound/return/count ledger posting, source-to-in-transit shipment, destination receipt, transfer discrepancies, quarantine, tolerances, row-locked reservations, partial-receipt follow-up work, derived states, warehouse work, RLS, audit and outbox. The installable warehouse PWA caches tasks in IndexedDB, queues version-bound idempotent transitions and physical quantities, replays them in order, blocks stale conflicts for review, and supports camera/hardware/manual barcode verification. | reporting, notifications, extended domain exceptions, offline receipt-line entry, putaway/pick/pack physical screens, transfer discrepancy resolution, and broader PWA device/browser acceptance |

...
| Operations | Phase 3 order/task foundation with atomic receiving, allocation, shipment, and RMA execution: valued inbound/outbound/return ledger posting, quarantine, tolerances, row-locked reservations, derived states, explicit inspection/refund/replacement/credit decisions, warehouse work, live queue, RLS, audit and outbox. The installable warehouse PWA caches tasks in IndexedDB, queues version-bound idempotent transitions, replays them in order, blocks stale conflicts for review, and supports camera/hardware/manual barcode verification. | reporting, extended domain exceptions, task-specific physical posting screens, and broader PWA device/browser acceptance |

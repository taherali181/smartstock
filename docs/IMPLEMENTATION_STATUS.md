...
| Operations | Phase 3 order/task foundation with atomic receiving, allocation, shipment, RMA, and task-bound blind-count execution: valued inbound/outbound/return/count ledger posting, quarantine, tolerances, row-locked reservations, derived states, explicit inspection/refund/replacement/credit decisions, warehouse work, live queue, RLS, audit and outbox. The installable warehouse PWA caches tasks in IndexedDB, queues version-bound idempotent transitions and counted quantities, replays them in order, blocks stale conflicts for review, and supports camera/hardware/manual barcode verification. | reporting, notifications, extended domain exceptions, remaining receive/putaway/pick/pack/transfer physical screens, and broader PWA device/browser acceptance |
